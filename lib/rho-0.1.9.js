...
BlockCompiler.prototype = {
  ...
  emitBlock: function(walk) {
    ...
    if (this.tryDiv3(walk)) return;
    if (this.tryDiv2(walk)) return;
  ...
  tryDiv2: function(walk) {
    ...
    while (b.hasCurrent())
      this.emitBlock(b);
    this.out.push("</div>\n");
    walk.startFrom(endIdx + 4).skipBlankLines();
    return true;
  },

  tryDiv3: function(walk) {
    if (!walk.at("~~~~~")) return false;
    walk.skip(5);
    var startIdx = walk.position;
    var endIdx = walk.indexOf("~/~~~");
    ...
    while (b.hasCurrent())
      this.emitBlock(b);
    this.out.push("</div>\n");
    walk.startFrom(endIdx + 5).skipBlankLines();
    return true;
  },

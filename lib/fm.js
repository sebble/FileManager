        ...
        */
        ...
        case 'text':
            ...
            html = '<script id="source" type="text/markdown">'+content+'</script><iframe></iframe>'
                 ...
                 + 'var html = "<head><meta charset="utf-8"><title>iframe</title>";'
                 + '    html = html + "<style>@import url(//my.sbl.io/raw/projects/file-browser/lib/md.css);</style>";'
                 + '    html = html + "</head><body>";'
                 + '    html = html + md.makeHtml($("#source").html());'
                 + '    html = html + "</body>";'
                 + '$("iframe").contents().find("html")[0].innerHTML = html;'
                 + '</script>';
            break;
        case 'book':
            content = fetch(url);
